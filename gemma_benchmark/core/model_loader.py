...
class BaseModelLoader:
    ...
    def __init__(self, model_name: str, config: Dict[str, Any]):
        ...
        self.config = config
        self.logger = logging.getLogger(f"gemma_benchmark.model_loader.{self.__class__.__name__}")
    ...
    def load(self) -> Tuple[Any, Any]:
        ...
        # Load tokenizer
        tokenizer = AutoTokenizer.from_pretrained(model_id, cache_dir=self.config.get("cache_dir"))
...
class HuggingFaceGenericLoader(BaseModelLoader):
    ...
    def _get_model_id(self) -> str:
        ...
        if not model_id:
            raise ValueError("model_id must be specified for 'huggingface' type models.")
        return model_id
...
class ModelManager:
    ...
    def load_model(self, model_name: str, config: Dict[str, Any]) -> ModelInterface:
        ...
        try:
            ...
            self.logger.info(f"Successfully loaded model and tokenizer for '{model_name}'.")
            wrapped_model = ModelWrapper(model, tokenizer, model_name)
            self._loaded_models[model_name] = wrapped_model
            return wrapped_model

        except torch.cuda.OutOfMemoryError as e:
            self.logger.error(f"GPU out of memory for model '{model_name}'. Try enabling quantization or using a smaller model.")
            raise ModelLoadingError(f"Out of Memory: {e}") from e

        except HfHubHTTPError as e:
            if e.response.status_code == 401:
                self.logger.error(f"Authentication failed for model '{model_name}'. Please check your HuggingFace token (HF_TOKEN).")
                raise ModelLoadingError("HuggingFace authentication failed.") from e
            else:
                self.logger.error(f"HTTP error when downloading model '{model_name}': {e.response.status_code}")
                raise ModelLoadingError(f"HTTP Error: {e}") from e

        except Exception as e:
            self.logger.error(f"An unexpected error occurred while loading model '{model_name}': {e}", exc_info=True)

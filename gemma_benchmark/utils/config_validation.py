...
class FlexibleModelConfig(BaseModel):
    ...
    model_id: str | None = None  # For HuggingFace models
    ...
    def validate_type(cls, v: str) -> str:
        if v.lower() not in [m.value for m in ModelType]:
            logging.getLogger(__name__).warning(
                f"Custom model type '{v}' used. Ensure a loader is available."
            )
        return v.lower()
...
class FlexibleTaskConfig(BaseModel):
    ...
    def validate_type(cls, v: str) -> str:
        if v.lower() not in [t.value for t in TaskType]:
            logging.getLogger(__name__).warning(
                f"Custom task type '{v}' used. Ensure a task class is registered."
            )
        return v.lower()
...
class FlexibleBenchmarkConfig(BaseModel):
    ...
    @classmethod
    def check_not_empty(cls, v: dict[str, Any], info: Any) -> dict[str, Any]:
        if not v:
...
class FlexibleConfigValidator:
    ...
    def validate_config_dict(
        ...
        try:
            ...
            raise ConfigurationError(
                f"An unexpected configuration error occurred: {e!s}"
            ) from e

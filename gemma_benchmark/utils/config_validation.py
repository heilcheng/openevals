...
try:
    ...
    logging.getLogger(__name__).error("Pydantic is required for this module. Please install with: pip install pydantic")
...
class FlexibleModelConfig(BaseModel):
    ...
    def validate_type(cls, v):
        ...
        if v.lower() not in [m.value for m in ModelType]:
            logging.getLogger(__name__).warning(f"Custom model type '{v}' used. Ensure a corresponding loader is available.")
        return v.lower()
...
class FlexibleTaskConfig(BaseModel):
    ...
    def validate_type(cls, v):
        ...
        if v.lower() not in [t.value for t in TaskType]:
            logging.getLogger(__name__).warning(f"Custom task type '{v}' used. Ensure a corresponding task class is registered.")
        return v.lower()
...
class FlexibleConfigValidator:
    ...
    def validate_config_dict(self, config_dict: Dict[str, Any]) -> FlexibleBenchmarkConfig:
        ...
        try:
            ...
            raise ConfigurationError(f"An unexpected configuration error occurred: {str(e)}")
    ...
    def validate_config_file(self, config_path: str) -> FlexibleBenchmarkConfig:
        ...
        try:
            ...
            if not isinstance(config_dict, dict):
                raise ConfigurationError("Configuration file is not a valid dictionary.")
            ...
            if isinstance(e, ConfigurationError):
                 raise
            raise ConfigurationError(f"Failed to process config file {config_path}: {e}")

...
try:
    ...
    logging.getLogger(__name__).warning(
        "seaborn not available - using matplotlib defaults"
    )
...
try:
    ...
    HAS_PANDAS = False
    logging.getLogger(__name__).warning(
        "pandas not available - some export features disabled"
    )
...
try:
    ...
    HAS_SCIPY = False
    logging.getLogger(__name__).warning(
        "scipy not available - statistical tests disabled"
    )
...
class BenchmarkVisualizer:
    ...
    def __init__(self, output_dir: str, style: str = "publication"):
        ...
        self.logger.info(
            f"Initialized visualizer with {style} style, output: {output_dir}"
        )
    ...
    def _save_figure(
        self, fig: plt.Figure, filename: str, formats: List[str] = None
    ) -> List[str]:
    ...
    def create_performance_overview(
        ...
        for model_results in results.values():
            tasks.update(
                [task for task in model_results.keys() if task != "efficiency"]
            )
        ...
        plt.suptitle(
            "Language Model Benchmark Performance Overview",
            fontsize=20,
            fontweight="bold",
        )
    ...
    def _create_performance_heatmap(
        self, ax: plt.Axes, results: Dict, models: List[str], tasks: List[str]
    ):
    ...
    def _create_model_ranking(
        ...
        bars = ax.barh(
            range(len(models_sorted)), scores_sorted, color=colors, alpha=0.8
        )
    ...
    def _create_task_difficulty(self, ax: plt.Axes, results: Dict, tasks: List[str]):
        ...
        for task in tasks:
            ...
            for model_results in results.values():
                if (
                    task in model_results
                    and "overall" in model_results[task]
                    and "accuracy" in model_results[task]["overall"]
                ):
    ...
    def _create_performance_distribution(
        ...
        for model in models:
            ...
            for task in tasks:
                if (
                    task in results[model]
                    and "overall" in results[model][task]
                    and "accuracy" in results[model][task]["overall"]
                ):
    ...
    def create_efficiency_analysis(
        ...
        self._create_performance_efficiency_tradeoff(
            ax4, results, models_with_efficiency
        )
    ...
    def _create_efficiency_frontier(self, ax: plt.Axes, efficiency_data: Dict):
        ...
        if model_efficiency:
            ...
            scatter = ax.scatter(
                memories, throughputs, c=efficiencies, s=100, cmap="viridis", alpha=0.8
            )
    ...
    def _create_performance_efficiency_tradeoff(
        self, ax: plt.Axes, results: Dict, efficiency_data: Dict
    ):
        ...
        for model_name in efficiency_data.keys():
            if model_name in results:
                ...
                for task, task_results in results[model_name].items():
                    if (
                        task != "efficiency"
                        and "overall" in task_results
                        and "accuracy" in task_results["overall"]
                    ):
                ...
                if accuracies:
                    ...
                    if "tokens_per_second" in eff_data:
                        avg_throughput = np.mean(
                            list(eff_data["tokens_per_second"].values())
                        )
    ...
    def _create_multi_run_analysis(self, multi_run_data: List[Dict]) -> List[str]:
        ...
        plt.suptitle(
            "Statistical Analysis (Multiple Runs)", fontsize=20, fontweight="bold"
        )
        plt.tight_layout()

        return self._save_figure(fig, "statistical_analysis")

    def _create_performance_with_ci(
        self,
        ax: plt.Axes,
        ...
        for model in models:
            ...
            for run_data in multi_run_data:
                if model in run_data:
                    ...
                    for task in tasks:
                        if (
                            task in run_data[model]
                            and "overall" in run_data[model][task]
                            and "accuracy" in run_data[model][task]["overall"]
                        ):
                            model_scores.append(
                                run_data[model][task]["overall"]["accuracy"]
                            )
                    if model_scores:
                ...
                if i != j:
                    significance = (
                        ...
                        if p_values[i, j] < 0.001
                            ...
                            else "*" if p_values[i, j] < 0.05 else ""
                        )
                    )
                    ax.text(
                        j,
    ...
    def _create_consistency_analysis(
        self,
        ax: plt.Axes,
        multi_run_data: List[Dict],
        models: List[str],
    ...
    def _create_single_run_analysis(
        self, results: Dict[str, Dict[str, Any]]
    ) -> List[str]:
    ...
    def _create_model_comparison_by_task(
        self, ax: plt.Axes, results: Dict, models: List[str], tasks: List[str]
    ):
    ...
    def _create_task_difficulty_ranking(
        self, ax: plt.Axes, results: Dict, tasks: List[str]
    ):
        ...
        for i, (bar, score) in enumerate(zip(bars, scores)):
            ...
            color = (
                "#2E8B57" if score > 0.7 else "#DAA520" if score > 0.4 else "#DC143C"
            )
            ax.text(
                score + 0.02,
                bar.get_y() + bar.get_height() / 2,
    ...
    def _create_performance_correlation(
        self, ax: plt.Axes, results: Dict, models: List[str], tasks: List[str]
    ):
    ...
    def export_results_summary(
        self, results: Dict[str, Dict[str, Any]], output_format: str = "json"
    ) -> str:
        """
        ...
        if output_format == "json":
            ...
            if not HAS_PANDAS:
                ...
                with open(output_path, "w") as f:
                    ...
                    # Write data
                    for model_name, model_results in summary[
                        "performance_summary"
                    ].items():
                        for task_name, task_results in model_results.items():
                ...
                for model_name, model_results in summary["performance_summary"].items():
                    for task_name, task_results in model_results.items():
                        row = {"model": model_name, "task": task_name, **task_results}
                        rows.append(row)
...
class ChartGenerator:
    ...
    def create_model_comparison_chart(
        ...
        try:
            ...
            for model in models:
                if (
                    "overall" in task_results[model][task_name]
                    and "accuracy" in task_results[model][task_name]["overall"]
                ):
    ...
    def create_efficiency_comparison_chart(
        self, results: Dict[str, Dict[str, Any]]
    ) -> Dict[str, str]:
    ...
    def create_subject_breakdown_chart(
        self, results: Dict[str, Dict[str, Any]], model_name: str, task_name: str
    ) -> str:
        """Create subject breakdown chart for MMLU-like tasks."""
        try:
            if (
                model_name not in results
                or task_name not in results[model_name]
                or "subjects" not in results[model_name][task_name]
            ):
            ...
            ax.set_xlabel("Accuracy")
            ax.set_title(
                f"{model_name} - {task_name.upper()} Subject Breakdown",
                fontweight="bold",
            )
            ax.set_xlim(0, 1.1)
            plt.tight_layout()
    ...
    def create_model_size_vs_performance_chart(
        self, results: Dict[str, Dict[str, Any]], task_name: str
    ) -> str:
        ...
        try:
            ...
            for model_name, model_data in results.items():
                if task_name in model_data and "overall" in model_data[task_name]:
                    ...
                    if size is not None:
                        ...
                        sizes.append(size)
                        performances.append(
                            model_data[task_name]["overall"]["accuracy"]
                        )
            ...
            ax.set_ylabel("Accuracy")
            ax.set_title(
                f"Model Size vs Performance on {task_name.upper()}", fontweight="bold"
            )
            ax.grid(True, alpha=0.3)
            plt.tight_layout()
...
def create_comprehensive_report(
    ...
    try:
        ...
        generated_files["statistical"] = visualizer.create_statistical_analysis(
            results, multi_run_data
        )
...
if __name__ == "__main__":
    ...
    example_results = {
        "gemma-2b": {
            ...
            "gsm8k": {"overall": {"accuracy": 0.45, "total": 500}},
            "efficiency": {
        ...
        "gemma-9b": {
            ...
            "gsm8k": {"overall": {"accuracy": 0.58, "total": 500}},
            "efficiency": {
        ...
        "mistral-7b": {
            ...
            "gsm8k": {"overall": {"accuracy": 0.52, "total": 500}},
            "efficiency": {
    ...
    report_files = create_comprehensive_report(
        example_results, output_dir, style="presentation"
    )

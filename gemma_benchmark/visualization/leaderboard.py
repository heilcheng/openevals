...
class LeaderboardGenerator:
    ...
    def _get_metric(self, task_result: Dict[str, Any], metric_name: str) -> Optional[float]:
        """
        ...
        """
        if task_result and "overall" in task_result and metric_name in task_result["overall"]:
    ...
    def _calculate_average_score(self, tasks: Dict[str, Any]) -> float:
        ...
        """
        scores = [
            score for task_name, metric in self.task_metrics.items()
    ...
    def generate_markdown_leaderboard(self, results: Dict[str, Dict[str, Any]]) -> str:
        ...
        for model_name, tasks in results.items():
            if not isinstance(tasks, dict):
                self.logger.warning(f"Skipping invalid result entry for model: {model_name}")
                continue
        ...
        header = "| Rank | Model | Average Score |" + " | ".join([t.upper() for t in header_tasks]) + " |"
        separator = "|:----:|:------|:--------------:|:" + ":|:".join(["---:"] * len(header_tasks)) + ":|"

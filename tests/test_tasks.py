...
class TestMMLUBenchmark:
    ...
    def test_load_data_success(self, mock_load_dataset):
        ...
        mock_dataset = {
            "test": {"subject": ["math"], "question": ["Q1"], "choices": [["A", "B"]], "answer": [0]},
            "dev": {"subject": ["math"], "question": ["Q_dev1"], "choices": [["A_dev", "B_dev"]], "answer": [1]}
        }
...
def test_mmlu_answer_extraction_robustness():
    ...
    mock_data = {
        "science": [{
            "question": "What is H2O?", "options": ["Water", "Air", "Fire", "Earth"],
            "answer": 0, "subject": "science", "examples": []
        }]
    }
    benchmark.data = mock_data
    
    test_cases = [
        ("A", 1), ("The answer is A", 1), ("(A)", 1), ("A.", 1),
        ("B", 0), # Incorrect case
    ]
...
def test_efficiency_token_counting():
    ...
    config = {
        "sample_prompts": ["Test"], "output_lengths": [128],
